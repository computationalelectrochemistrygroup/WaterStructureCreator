...
from .modified_pymatgen.substrate_analyzer_mod import reduce_vectors
...
def get_maximally_isotropic_cell(matrices, method='PCA'):
    ...
    tuple of dimension 3
        ...
        list of 5 elements with information of the optimization method, list[1] matrix 2x2.
        list of all matrices sorted by stdev_EV

    """
    #opt_2D, allmatrices_2D = analyse_voronoi(matrices, method=method)
    allmatrices_2D = analyse_voronoi(matrices, method=method)
    allmatrices_2D = sorted(allmatrices_2D, key=lambda x: x[3])

    # The matrices are already sorted, no need for the other output
    #return opt_2D[2], opt_2D, allmatrices_2Dsorted
    return(allmatrices_2D)
...
def analyse_lattice_points_kmesh_voronoi(recarr_maybereduced):
    """Short summary.
...
def analyse_lattice_points_kmesh_voronoi_PCA(recarr_maybereduced):
